...
private fun Content(
    navController: NavController,
    viewModel: AddPregnancyViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(key1 = context) {
        viewModel.validationEvents.collect { event ->
            when (event) {
                is AddPregnancyViewModel.ValidationEvent.Success -> {
                ...
                is AddPregnancyViewModel.ValidationEvent.Failed -> {
                    Toast.makeText(context, event.message, Toast.LENGTH_LONG).show()
                }
            }
        }
        ...
        Box(
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = {
                        viewModel.onEvent(AddPregnancyFormEvent.DateChanged(it))
                    },
                    modifier = Modifier
                        .fillMaxWidth()
                        .focusRequester(focusRequester),
                    isError = pregnancyFormState.dateError != null,
                    label = { Text(text = "Perkiraan Tanggal Kehamilan") },
                    placeholder = { Text(text = "Masukkan perkiraan tanggal kehamilan") },
                    supportingText =
                    pregnancyFormState.dateError?.let {
                        { Text(text = pregnancyFormState.dateError) }
                    } ?: run {
                        { Text(text = "Hari dari tanggal kehamilan tidak perlu tepat") }
                    },
                ...
                Button(
                    onClick = { viewModel.onEvent(AddPregnancyFormEvent.Submit) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(top = 8.dp),
                ) {
    ...
    CalendarDialog(
        ...
        selection = CalendarSelection.Date { date ->
            viewModel.onEvent(AddPregnancyFormEvent.DateChanged(date.toString()))
