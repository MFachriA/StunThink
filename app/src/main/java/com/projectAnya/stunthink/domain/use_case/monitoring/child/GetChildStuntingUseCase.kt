...
class GetChildStuntingUseCase(
    private val userRepository: UserRepository
) {
    operator fun invoke(token: String, id: String): Flow<Resource<List<Stunting>>> = flow {
        try {
            emit(Resource.Loading())
            val childList = userRepository.getChildStunting(token = token, id = id)
